'''car has name,model,type,speed'''
class Car(object):#Default name:general, model:GM '''
  ...
  def __init__(self, name='General', model='GM', vehicle_type=None,speed = 0):
    ...
    listtwodoors= ['Porshe', 'Koenigsegg']
  ...
  #  def numofdoors():
    if self.name not in listtwodoors : #Porshe or Koenigsegg two doors else 4
        self.num_of_doors = 4
    else:
        self.num_of_doors = 2
  ...
  #  def wheels():
    if self.vehicle_type is manywheels:
